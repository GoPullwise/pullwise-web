    ...
    <div className="landing fade-in">
      <header className="lp-top">
        ...
        <nav className="lp-nav">
          ...
          <button className="btn ghost sm" onClick={() => go("security")}>{T("Security", "安全")}</button>
      ...
      <div className="legal-shell">
        <aside className="legal-side">
          ...
          {sections.map((section) => (
            <a key={section.id} className="legal-side-i" href={`#${section.id}`}>{section.title}</a>
          ))}
        ...
        <main className="legal-main">
          ...
          <div className="legal-foot-actions">
            <span className="muted">{T("Questions? Email support@pullwise.dev.", "如有问题，请联系 support@pullwise.dev。")}</span>
          </div>
        </main>
      </div>
    </LegalChrome>
...
function Section({ id, title, children }) {
  return (
    <>
      <h2 className="legal-h2" id={id}>{title}</h2>
      {children}
    </>
  );
}
  ...
  return (
    ...
    >
      <Section id="data" title={sections[0].title}>
        <p>{T("We collect your email address, GitHub profile metadata, authorized repository metadata, scan records, issue findings, and basic operational logs.", "我们会收集邮箱地址、GitHub 资料元数据、已授权仓库元数据、扫描记录、问题发现和基础运行日志。")}</p>
      </Section>
      <Section id="code" title={sections[1].title}>
        <p>{T("Repository contents are cloned by the backend worker only for the scan you start. Source code is not stored in the browser and is not used to train models by Pullwise.", "仓库内容仅由后端 worker 为你启动的扫描而克隆。源代码不会存入浏览器，Pullwise 不会将你的代码用于模型训练。")}</p>
      </Section>
      <Section id="payments" title={sections[2].title}>
        <p>{T("Payments are processed by the enabled provider, Stripe or Creem. Pullwise stores provider customer and subscription identifiers, but does not store card numbers.", "支付由启用的 Stripe 或 Creem 处理。Pullwise 只保存支付平台的客户和订阅标识，不保存银行卡号。")}</p>
      </Section>
      <Section id="retention" title={sections[3].title}>
        <p>{T("Account and subscription metadata is kept while your account is active. Scan findings may be retained for product operation and can be deleted when you close your account.", "账户和订阅元数据会在账户有效期间保留。扫描发现会为产品运行而保留，并可在关闭账户时删除。")}</p>
      </Section>
      <Section id="rights" title={sections[4].title}>
        <p>{T("You can request export, correction, or deletion of your account data by contacting support@pullwise.dev.", "你可以通过 support@pullwise.dev 请求导出、更正或删除账户数据。")}</p>
      </Section>
    </LegalDocLayout>
  );
}
...
export function TermsScreen({ go, auth }) {
  ...
  return (
    <LegalDocLayout
      ...
      subtitle={T("These terms govern your use of Pullwise repository review and billing features.", "这些条款适用于 Pullwise 仓库审查和支付功能。")}
    >
      <Section id="service" title={sections[0].title}>
        <p>{T("Pullwise provides GitHub-connected code review workflows. Findings are recommendations and must be reviewed by you before acting on them.", "Pullwise 提供连接 GitHub 的代码审查流程。审查发现仅为建议，你需要自行确认后再采取行动。")}</p>
      </Section>
      <Section id="account" title={sections[1].title}>
        <p>{T("You are responsible for keeping your GitHub account, email inbox, and Pullwise sessions secure.", "你需要负责保护 GitHub 账户、邮箱和 Pullwise 会话的安全。")}</p>
      </Section>
      <Section id="billing" title={sections[2].title}>
        <p>{T("Subscriptions are billed by the configured provider, Stripe or Creem. You can manage or cancel an active subscription from the billing page when a customer portal is available.", "订阅由配置的 Stripe 或 Creem 收费。当支付平台提供客户门户时，你可以从支付页面管理或取消订阅。")}</p>
      </Section>
      <Section id="limits" title={sections[3].title}>
        <p>{T("Do not use Pullwise to scan repositories you are not authorized to access, to process illegal content, or to attack GitHub, payment providers, or Pullwise infrastructure.", "不得使用 Pullwise 扫描未授权仓库、处理违法内容，或攻击 GitHub、支付平台及 Pullwise 基础设施。")}</p>
      </Section>
      <Section id="liability" title={sections[4].title}>
        <p>{T("Pullwise is provided as a software service. To the maximum extent permitted by law, Pullwise is not liable for indirect or consequential damages.", "Pullwise 作为软件服务提供。在法律允许的最大范围内，Pullwise 不承担间接或后果性损失责任。")}</p>
      </Section>
    </LegalDocLayout>
  );
}
  ...
  const controls = [
    {
      ...
      text: T("Pullwise requests read-only repository access for metadata and code checkout unless you add future write features.", "Pullwise 默认只请求只读仓库权限，用于元数据读取和代码 checkout。"),
    },
    {
      icon: <I.Lock size={18} />,
      title: T("Secret isolation", "密钥隔离"),
      text: T("OAuth secrets, GitHub App private keys, payment API keys, and Codex credentials stay on the backend.", "OAuth 密钥、GitHub App 私钥、支付 API key 和 Codex 凭据只保存在后端。"),
    },
    {
      icon: <I.Terminal size={18} />,
      title: T("Read-only review runner", "只读审查运行器"),
      text: T("Codex review runs against a backend checkout with read-only sandbox settings and emits structured findings.", "Codex 审查在后端 checkout 中以只读沙箱运行，并输出结构化发现。"),
    },
    {
      icon: <I.Database size={18} />,
      title: T("Server-backed state", "服务端状态"),
      text: T("Sessions, repository authorization, scans, findings, and billing status are persisted by the backend, not browser fixtures.", "会话、仓库授权、扫描、发现和支付状态由后端持久化，而不是浏览器假数据。"),
    },
  ];
    ...
    <LegalChrome go={go} current="security" auth={auth}>
      <section className="security-hero">
        ...
        <h1 className="lp-title">
          {T("A conservative security baseline", "保守可靠的安全基线")}<br />
          <span className="lp-title-em">{T("for GitHub-connected review.", "用于连接 GitHub 的代码审查。")}</span>
        </h1>
        <p className="lp-sub">
          {T("This page describes implemented controls only. Pullwise does not claim third-party certifications unless they are explicitly published here later.", "本页只描述已实现的控制措施。除非未来在此明确发布，Pullwise 不声明第三方合规认证。")}
        </p>
      </section>
      ...
      <section className="security-section">
        <div className="security-section-h">
          ...
          <p>{T("Controls that exist in the product architecture today.", "当前产品架构中已经存在的控制措施。")}</p>
        </div>
      ...
      <section className="lp-cta-band">
        ...
        <button className="btn primary lg"><I.Mail /> security@pullwise.dev</button>
      </section>
    </LegalChrome>
  );
}
    ...
    <LegalChrome go={go} current="status" auth={auth}>
      <section className="status-hero">
        ...
        <p className="status-sub">
          {T("Last checked", "最近检查")} {now.toLocaleTimeString()} · {T("Reads live /health data every 30s", "每 30 秒读取实时 /health 数据")}
        </p>
      </section>
      ...
      <section className="status-section">
        <div className="status-card card">
          <div className="status-card-h">
            ...
            <span className="muted">{T("No generated uptime or incident history", "无生成的 uptime 或事故历史")}</span>
          </div>
          <StatusRow icon={<I.Code size={14} />} title={T("Web app", "Web 应用")} status="operational" detail={window.location.host || "local browser"} />
          <StatusRow icon={<I.Activity size={14} />} title={T("REST API", "REST API")} status={apiStatus} detail={apiDetail} />
          <StatusRow icon={<I.Database size={14} />} title={T("State database", "状态数据库")} status={health?.database ? "operational" : apiStatus} detail={databaseDetail} />
          {error && (
