...
function Sparkline({ data, color, w = 180, h = 36 }) {
  ...
  const d = points.map((point, index) => (index === 0 ? `M${point[0]},${point[1]}` : `L${point[0]},${point[1]}`)).join(" ");
  return (
        ...
        <div className="main" style={{ maxWidth: "none" }}>
          <div className="page-h">
            <div>
              ...
              <div className="sub" style={{ display: "flex", alignItems: "center", gap: 6, flexWrap: "wrap" }}>
                {latestScan ? (
                  <>
                    <span className="tag"><I.GitBranch size={10} /> {latestScan.branch}</span>
                    <span className="tag">{latestScan.commit}</span>
                    <span>{T(`Last scan: ${latestScan.time}`, `最近扫描：${latestScan.time}`)}</span>
                  </>
                ) : (
          ...
          <div className="kpi-row">
            <div className="kpi card">
              <div className="kpi-h"><span className="kpi-l">{T("Open issues", "未解决问题")}</span></div>
            ...
            <div className="kpi card">
              ...
              <div className="kpi-v" style={{ color: "var(--sev-critical)" }}>{counts.critical}</div>
              <div className="kpi-foot">{T("From stored scan results", "来自已保存扫描结果")}</div>
            </div>
            <div className="kpi card">
              <div className="kpi-h"><span className="kpi-l">{T("Authorized repos", "已授权仓库")}</span></div>
              <div className="kpi-v">{reposLoading ? "-" : repositories.length}</div>
              <div className="kpi-foot">{needsAuthorization ? T("GitHub authorization required", "需要 GitHub 授权") : T("GitHub App access", "GitHub App 权限")}</div>
            </div>
            <div className="kpi card">
              <div className="kpi-h"><span className="kpi-l">{T("Scans", "扫描")}</span></div>
              <div className="kpi-v">{scansLoading ? "-" : scans.length}</div>
              <div className="kpi-foot">{T("Persisted in server SQLite", "已持久化到 server SQLite")}</div>
            </div>
          </div>
          ...
          <div className="dash-grid">
            <div className="card dash-summary">
              <div className="dash-summary-head">
                ...
                <button className="btn sm" onClick={() => go("issues")}>{T("All issues", "所有问题")} <I.ArrowR size={12} /></button>
              </div>
            ...
            <div className="card dash-cats">
              <div className="dash-summary-head"><h3>{T("Repositories", "仓库")}</h3></div>
              {repositories.slice(0, 7).map((repo) => (
                <div key={repo.id} className="dash-cat-row">
                  ...
                  <div className="dash-cat-bar"><div style={{ width: "100%", background: "var(--accent)" }}></div></div>
                  <b style={{ minWidth: 18, textAlign: "right" }}>{repo.private ? <I.Lock size={12} /> : ""}</b>
                </div>
              ))}
          ...
          <div className="dash-issues-h">
            <div>
              ...
              <div className="sub" style={{ fontSize: 12.5 }}>
                {issuesLoading ? T("Loading issues", "正在加载问题") : T(`${openIssues.length} open issues`, `${openIssues.length} 个未解决问题`)}
              </div>
            </div>
            <button className="btn sm" onClick={() => go("issues")}>{T("All issues", "所有问题")} <I.ArrowR size={12} /></button>
          </div>
          ...
          {!issuesLoading && openIssues.length === 0 && !issuesError && (
            <div className="card section muted">
              {T("No findings have been written by the scan worker yet.", "扫描 worker 尚未写入任何 finding。")}
            </div>
          )}
          {layout === "list" && openIssues.length > 0 && (
            <div className="issue-list">
              {openIssues.slice(0, 8).map((issue) => (
                <IssueRow key={issue.id} issue={issue} onClick={() => { setIssue(issue); go("issue"); }} />
              ))}
            </div>
          )}
        </div>
